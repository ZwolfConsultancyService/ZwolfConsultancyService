...
const BlogDetail = () => {
    ...
    const loadBlogData = async () => {
      try {
        ...
        if (blogs) {
          ...
          // ✅ Primary: URL slug se match karo (direct URL / new tab ke liye)
          const blogBySlug = blogs.find((b) => createSlug(b.title) === slug);
          if (blogBySlug) {
            setBlog(blogBySlug);
          } else if (storedBlog) {
            // Fallback: sessionStorage se title match karo
  ...
  const handleNavigation = (targetBlog) => {
    ...
    const targetSlug = createSlug(targetBlog.title);
  ...
  if (!blog) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-600 text-xl mb-4">Blog not found.</p>
          <button
